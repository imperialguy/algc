...
class GLMModel(object):
    ...
    def __init__(self, coefficients_dataframe):
        ...
        self.intercept = coefficients_dataframe[
                coefficients_dataframe['feature'] == '(Intercept)'].squeeze()['coefficient']
    ...
    def score_data(self, obs):
        ...
        Return:
            ...
            a pandas DataFrame with the contributions and all other feature data
            as the 1th element
        ...
        coeff_dataframe = pandas.DataFrame(data={'coefficient': self.intercept,
                                                 'xi': 1,
                                                 'contribution': self.intercept
                                                 }, index=['(Intercept)'])

        return (self.intercept + scoring_data['contribution'].sum(),
                pandas.concat([scoring_data.set_index('feature'), coeff_dataframe])
                )
    ...
    def create_rule(self, feature, rule_function):
        ...
        try:
            # python 3.x
            number_of_args = len(inspect.signature(rule_function).parameters)
        except AttributeError:
            # python 2.x
            number_of_args = len(inspect.getargspec(rule_function)[0])

        if number_of_args != 2:
            raise ValueError('Arg specification for rule_function \
                should be func(arg1, arg2)')

        self.__rules[feature] = rule_function
    ...
    def load_rules(self, rules_dict):
        ...
        for key, value in rules_dict.items():
            ...
            try:
                # python 3.x
                number_of_args = len(inspect.signature(value).parameters)
            except AttributeError:
                # python 2.x
                number_of_args = len(inspect.getargspec(value)[0])
            if not callable(value) or number_of_args != 2:
                raise ValueError('Arg specification for rule_function \
                    should be func(arg1, arg2)')
    ...
    def prep_data_and_score(self, obs):
        ...
        for idx, column in enumerate(self.__features):
            model_data.loc[idx] = [column, self.__rules[column](obs,
                    self.__coefficients[
